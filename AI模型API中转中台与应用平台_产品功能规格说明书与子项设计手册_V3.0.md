# AI 模型 API 中转中台与应用平台
## 产品功能规格说明书与子项设计手册 (FRS V3.0 终极整合版)
### —— 面向业务决策者与非技术客户
...
本说明书针对本token 聚合平台的业务决策者、客户及产品运营团队编写，从**解决用户核心痛点、原始需求精准对照、交互界面、业务流程与管理功能**出发，对项目中的 **10 大核心功能模块、共 60 个细分功能子项** 进行逐一设计与说明。

为了让采购方与技术评审专家清晰了解每一项工时的来源与评估合理性，本文档对全量 60 个子项均给出了**【评估计算思路】**。特别是针对 **`【sub2api 匹配度】：部分满足`的子项**，采用了**三段式对比推导**：
...
### 2.2 一键部署客户端（Windows / macOS 专属客户端）（共需 25.0 人日）
...
- **功能描述**：支持用户在本平台客户端内部预设并保存多套配置预设卡片（包含不同的平台 Key、线路节点与监听端口）。滑动开关即可秒级无感切换当前生效的本平台线路配置。*（注：专属于本平台内部线路与 Key 切换，不涉及第三方平台）*
...
> **模块定位**：产品对外展示与获客的官方站点，承担品牌介绍、模型报价、使用教程与用户转化的核心职能。

#### 【子项 2.4.1 首页：产品核心功能介绍、价值主张、CTA 引导】
- **解决的痛点**：缺少官方获客阵地与清晰的价值主张传递，导致潜在客户流失率高。
- **功能描述**：展示产品优势（高可用、低延时、智能降本）与免费试用 CTA 引导入口，采用响应式布局。
- **sub2api 匹配度**：**完全缺失 (全新研发)**。
- **工时与岗位**：**4.0 人日** (产品: 1.0, 前端: 3.0)。
- **评估计算思路**：前端 3.0 天编写响应式动态 Landing Page，产品 1.0 天设计文案与 UI 原型，合计 4.0 人日。

#### 【子项 2.4.2 模型广场：支持的模型列表、能力说明与阶梯报价】
- **解决的痛点**：平台支持的模型能力与 Token 阶梯报价不透明，影响客户采购决策。
- **功能描述**：公开展示平台支持的模型列表、上下文长度、能力说明标签及实时 Token 阶梯报价。
- **sub2api 匹配度**：**部分满足**。
- **已满足的基础与实现机制**：`sub2api` 后端数据库记录了模型名称与上游基础费率倍率。
- **需二次改造的差距与研发内容**：缺少公开免登录的模型广场 UI 页面、交互式 Token 计算器、模型能力标签（如 128k 上下文、Vision、Code）以及阶梯充值折扣展示。
- **工时与岗位**：**3.0 人日** (后端: 1.0, 前端: 2.0)。
- **评估计算思路**：借用后端数据源，评估 3.0 人日快速搭建对外展示的模型广场与动态搜索墙。

#### 【子项 2.4.3 文档中心：安装教程、使用指南、API 文档】
- **解决的痛点**：缺乏结构化接入教程，导致技术客服日常解答重复问题负担极重。
- **功能描述**：结构化响应式知识库与文档中心站点，包含客户端安装教程、使用指南、API 接入步骤及 FAQ。
- **sub2api 匹配度**：**部分满足**。
- **已满足的基础与实现机制**：开源社区提供了零散的接口接入说明文档。
- **需二次改造的差距与研发内容**：缺乏结构化的商业级产品文档站点、客户端部署一键配置指引、在线 API 试用调试器以及 FAQ 分类。需搭建结构化文档与帮助中心站点并整理完善中文教程。
- **工时与岗位**：**3.0 人日** (产品: 1.0, 前端: 2.0)。
- **评估计算思路**：评估 3.0 人日完成静态文档站点工程化建站与内容编译。

#### 【子项 2.4.4 定价页：套餐对比与购买入口】
- **解决的痛点**：缺乏透明的套餐对比与在线直购充值入口，付费转化路径长。
- **功能描述**：展示套餐对比表格（入门版、专业版、企业版）并提供在线购买充值入口。
- **sub2api 匹配度**：**部分满足**。
- **已满足的基础与实现机制**：后端支持组别 (Group) 基础费率设置。
- **需二次改造的差距与研发内容**：官网缺乏可视化套餐对比矩阵表（入门版/专业版/企业版），缺乏直观的包月/按量购买入口与一键支付调起页面。
- **工时与岗位**：**3.0 人日** (产品: 1.0, 后端: 1.0, 前端: 1.0)。
- **评估计算思路**：评估 3.0 人日实现前台 Pricing 矩阵与充值单生成逻辑。

#### 【子项 2.4.5 关于/联系我们：企业信息与客服联系方式】
- **解决的痛点**：缺少企业背书与售后服务通道，客户缺乏信任感。
- **功能描述**：展示企业信息、服务条款、隐私政策及客服联系方式（微信/Telegram/工单）。
- **sub2api 匹配度**：**完全缺失 (全新研发)**。
- **工时与岗位**：**2.0 人日** (前端: 2.0)。
- **评估计算思路**：前端 2.0 天开发联系页面、富文本条款展示与在线工单唤起表单，合计 2.0 人日。

---

### 2.5 核心模型网关能力补全（共需 54.5 人日）

> **模块定位**：在基础转发能力之上，补齐网关的智能化调度、亲和性隔离、语义缓存与成本极致优化能力。本模块为网关研发的核心重置区。

#### 【子项 2.5.1 最高性价比调度策略：基于上游号池实时价格、可用额度、模型能力等多维度，自动选择最优渠道路由】
- **解决的痛点**：盲目调度高价通道导致平台 Token 采购成本居高不下，运营毛利润低。
- **功能描述**：结合上游号池的实时价格、可用额度与延时，算法自动计算综合得分，选择单位成本最低且平稳的通道路由。
- **sub2api 匹配度**：**不满足 (二次开发重点)**。
- **工时与岗位**：**12.0 人日** (产品: 2.0, 后端: 8.5, 前端: 1.5)。
- **评估计算思路**：后端需 8.5 天在调度器中植入基于实时汇率、历史响应延迟、成功率及账号剩余额度的多维滑动窗口加权算分引擎；前端 1.5 天渲染可视化算分策略配置；产品 2.0 天校验算分模型准确度，合计 12.0 人日。

#### 【子项 2.5.2 渠道专属号池绑定与亲和性路由】
- **解决的痛点**：大客户与普通客户共享号池，VIP 客户容易受到并发挤占干扰；代理商无法绑定专属资源。
- **功能描述**：当请求来自特定代理商或企业租户时，网关底层调度引擎优先且仅在其绑定的“专属号池”中调度节点，保障 VIP 资源物理/逻辑隔离。
- **sub2api 匹配度**：**不满足 (全系统技术难度最高攻坚点)**。
- **工时与岗位**：**18.0 人日** (产品: 3.0, 后端: 12.0, 前端: 3.0)。
- **评估计算思路**：后端需 12.0 天重构网关底层调度引擎与路由模块，编写高并发亲和路由表查找与 Tenant/Agent 专属池零拷贝转发；前端 3.0 天开发专属号池物理隔离勾选与绑定组件；产品 3.0 天进行高并发跨租户物理隔离验证，合计 18.0 人日。

#### 【子项 2.5.3 用户端无感切换：上游渠道故障/限流时，自动降级切换至备用渠道，用户侧无感知】
- **解决的痛点**：上游接口报错或触发 429 限流时直抛错误，导致终端用户体验恶劣。
- **功能描述**：通道报错或 429 限流时，秒级熔断并静默重试切换至备用渠道，用户侧无感知。
- **sub2api 匹配度**：**部分满足**。
- **已满足的基础与实现机制**：`sub2api` 原生已具备基础的单一渠道重试机制。
- **需二次改造的差距与研发内容**：无法针对 429/500/503 模型报错进行自动降级熔断与多备用渠道切换。
- **工时与岗位**：**3.0 人日** (后端: 2.5, 前端: 0.5)。
- **评估计算思路**：后端 2.5 天升级熔断状态机与备用渠道降级逻辑，前端 0.5 天展示降级告警状态，合计 3.0 人日。

#### 【子项 2.5.4 缓存降本策略：高频重复请求的语义缓存命中，减少上游调用量】
- **解决的痛点**：大量高频重复提问（如常见 FAQ）反复消耗高昂的上游 Token 费用。
- **功能描述**：不同用户提问高度相似问题时（相似度 > 95%），直接从本地 Vector 缓存返回，节约 100% 上游 Token。
- **sub2api 匹配度**：**不满足 (算法复杂度极高攻坚点)**。
- **已满足的基础与实现机制**：`sub2api` 原生仅支持简单的精确 Key-Value 字符串缓存。
- **需二次改造的差距与研发内容**：无法识别相似提问。需引入向量 Embedding 提取引擎、构建高性能向量数据库检索（>95% 相似度自动命中）以及防止高并发下缓存击穿失效与缓存更新机制。
- **工时与岗位**：**16.0 人日** (产品: 2.0, 后端: 11.0, 前端: 3.0)。
- **评估计算思路**：后端 11.0 天攻坚向量索引、Embedding 提取与高并发相似度检索；前端 3.0 天开发缓存命中率大盘与白名单配置；产品 2.0 天测试语义匹配精准度，合计 16.0 人日。

#### 【子项 2.5.5 失败重试与退避机制及优先级队列】
- **解决的痛点**：高并发拥堵时无法优先保证付费 VIP 响应，造成关键客户流失。
- **功能描述**：拥堵时优先保证付费 VIP 请求响应，结合智能指数退避重试策略，规避流量雪崩。
- **sub2api 匹配度**：**部分满足**。
- **已满足的基础与实现机制**：`sub2api` 原生自带极简的排队等待队列。
- **需二次改造的差距与研发内容**：缺乏基于租户权重的加权公平队列（WFQ）与令牌桶/漏桶指数退避流控算法。
- **工时与岗位**：**5.5 人日** (产品: 1.5, 后端: 3.0, 前端: 1.0)。
- **评估计算思路**：后端 3.0 天开发 WFQ 队列与指数退避引擎，产品 1.5 天验证拥堵倾斜，前端 1.0 天展示队列等待 UI，合计 5.5 人日。
...
- **解决的痛点**：海外客户看北京时间和 RMB 结算，容易产生财务账目混淆与争议。
- **功能描述**：显示时间自动适配用户当地时区，账单金额支持在 RMB (¥) 与 USD ($) 之间按汇率实时换算展示。
- **sub2api 匹配度**：**部分满足**。
- **已满足的基础与实现机制**：后端数据库按 UTC 标准时间戳存储日志与金额。
- **需二次改造的差距与研发内容**：前端固定按 UTC+8 显示时间与 RMB 结算。需增加基于用户浏览器/账号设置的时区自动转换与 USD/RMB 实时汇率换算显示组件。
- **工时与岗位**：**2.0 人日** (后端: 1.0, 前端: 1.0)。
- **评估计算思路**：评估 2.0 人日对接汇率 API 并编写前端格式化 Pipeline。
...
#### 【子项 2.10.6 结算与提现：阶梯销售佣金自动计算与结算流程】
- **解决的痛点**：缺乏灵活的阶梯销售激励机制，代理佣金依赖线下手工对账与人工转账，效率低且易出错。
- **功能描述**：支持为代理商配置阶梯销售佣金规则（按月消费额/ Token 消耗区间设定差异化佣金比例或固定奖励金额），系统定时自动穿透计算应得佣金，代理在线发起提现申请，平台管理员在线审核，线下打款。
- **sub2api 匹配度**：**完全缺失 (全新研发)**。
- **工时与岗位**：**5.0 人日** (产品: 1.0, 后端: 2.5, 前端: 1.5)。
- **评估计算思路**：后端 2.5 天编写阶梯销售佣金计算引擎、区间规则匹配与提现申请（在线审核、线下打款）状态机；前端 1.5 天开发可视化阶梯佣金规则配置 UI 与提现记录面板；产品 1.0 天设计阶梯算分模型与验收，合计 5.0 人日。
...
以下为 11 大核心模块及集成测试（共 60 个功能子项）在 3 人团队中的终极工时分配矩阵：

| 模块编号 | 模块名称与核心交付范围 | 子项数 | 产品工时 | 后端工时 | 前端工时 | 模块总工时 | 报价占比 |
| :---: | :--- | :---: | :---: | :---: | :---: | :---: | :---: |
| **2.1** | **中转站中台** (网关核心与熔断限流 P95≤50ms) | 5 项 | 0.5 人日 | 2.5 人日 | 0.5 人日 | **3.5 人日** | 1.57% |
| **2.2** | **一键部署客户端** (Win/Mac 双端专属客户端) | 6 项 | 6.0 人日 | 4.5 人日 | 14.5 人日| **25.0 人日**| 11.21% |
| **2.3** | **网页版对话机器人** (SaaS 对话与场景标签) | 7 项 | 6.0 人日 | 5.0 人日 | 12.0 人日| **23.0 人日**| 10.31% |
| **2.4** | **官网门户** (品牌/模型广场/文档中心) | 5 项 | 3.0 人日 | 2.0 人日 | 10.0 人日| **15.0 人日**| 6.73% |
| **2.5** | **核心模型网关能力补全** (亲和路由/性价比调度/向量缓存) | 5 项 | 9.5 人日 | 37.0 人日| 8.0 人日 | **54.5 人日**| 24.44% |
| **2.6** | **账单与充值系统** (零误差对账/日周月账单/充值) | 6 项 | 2.0 人日 | 8.5 人日 | 4.0 人日 | **14.5 人日**| 6.50% |
| **2.7** | **账号与权限体系** (四级多租户/2FA/子账号) | 6 项 | 7.0 人日 | 14.5 人日| 9.0 人日 | **30.5 人日**| 13.68% |
| **2.8** | **上游号池渠道管理** (监控大盘/告警/健康度) | 6 项 | 0.5 人日 | 2.5 人日 | 1.5 人日 | **4.5 人日** | 2.02% |
| **2.9** | **国际化（多语言）** (中英无缝/i18n架构/扩展) | 4 项 | 0.5 人日 | 2.0 人日 | 5.5 人日 | **8.0 人日** | 3.59% |
| **2.10**| **下游代理管理系统** (分销/专属号池/阶梯佣金) | 6 项 | 6.0 人日 | 11.5 人日| 7.5 人日 | **25.0 人日**| 11.21% |
| **2.11**| **全系统集成与高并发压测** (高吞吐/SLA) | 4 项 | 8.0 人日 | 6.5 人日 | 5.0 人日 | **19.5 人日**| 8.74% |
| **合计**| **223 人日精确定算** | **60 项**| **49.0 人日**| **96.5 人日**| **77.5 人日**| **223.0 人日**| **100%** |
...
为了满足客户“**T+28d (1个月) 内上线 P0 核心 MVP 可运营版本、T+49d (阶段二) 交付全终端应用、T+63d (阶段三) 完成 P1 官网门户、国际化与下游代理管理系统终验交付**”的商业节点要求，团队采用 3 人全职并行敏捷开发模式（1 PM/QA + 1 高级后端 BE + 1 高级前端 FE）。

> **时间轴说明**：**`T`**（即 **`T+0`**）代表外包项目合同生效与开发启动日；**`T+N`** 代表项目启动后的第 N 个自然日。

```mermaid
gantt
    title 223个人日外包项目 3人全职团队 T+N 相对工期甘特图 (全项目共 74 自然天)
    dateFormat YYYY-MM-DD
    axisFormat 第%j天

    section 阶段一 P0级MVP上线与500QPS压测 (84人日 | 需28天 | T1d至T28d)
    多租户与账号权限与2FA T1至T14d        :p1_rbac, 2026-01-01, 14d
    网关核心与性价比调度与熔断 T8至T20d    :p1_gw,   2026-01-08, 12d
    账单系统与微信支付宝充值 T15至T24d    :p1_pay,  2026-01-15, 10d
    网关500QPS压测调优与MVP联调 T22至T28d  :p1_stress, 2026-01-22, 7d
    M1 MVP商业上线与高并发验收 T28d        :crit, active, p1_m, 2026-01-28, 1d

    section 阶段二 P0级双端终端与算力攻坚 (82人日 | 需27天 | T29d至T55d)
    专属号池亲和路由与向量语义缓存 T29至T45d:p2_eng,  2026-01-29, 17d
    Win与Mac桌面客户端研发 T29至T48d       :p2_desktop, 2026-01-29, 20d
    WebChat对话机器人与多模态 T36至T53d    :p2_chat, 2026-02-05, 18d
    M2 阶段二终端成果验收与发布 T55d        :crit, active, p2_m, 2026-02-24, 1d

    section 阶段三 P1级代理分销与官网与交付 (57人日 | 需19天 | T56d至T74d)
    下游代理管理系统与阶梯佣金 T56至T68d    :p3_agent, 2026-02-25, 13d
    官网门户与文档中心与全站多语言 T60至T71d:p3_portal, 2026-03-01, 12d
    部署脚本与运维监控与终验交付 T66至T74d  :p3_deploy, 2026-03-07, 9d
    M3 项目终验与全系统交付 T74d            :crit, active, p3_m, 2026-03-15, 1d
```

### 各阶段交付范围与里程碑建议

#### 1. 阶段一：P0 级 MVP 极速上线运营与高并发压测 (T+1d ~ T+28d，需 28 天，共 84.0 人日)
- **优先级定位**：**P0 (核心必做)** + **P2 (角色强制 2FA)**
- **交付目标**：在 **T+28d (第 4 周末)** 内快速上线具备商业闭环能力且经受过 **500 QPS 压测调优** 的 MVP 运营版本，支持用户注册登录、四级分层多租户、网关核心调度、500 QPS 高并发防爆、在线充值与上游号池接驳。
- **交付功能模块与子项清单 (与第 2 章节序号精准对应)**：
  ...
  - **模块 2.5 核心模型网关 (基础调度与防护)** (小计 20.5d) [P0]
    - `2.5.1 最高性价比调度策略` (12.0d) / `2.5.3 用户端无感切换与降级` (3.0d) / `2.5.5 失败重试与加权公平队列(WFQ)` (5.5d)
  ...
  - **模块 2.7 账号与权限体系 (四级多租户/安全)** (共 30.5d) [P0/P2]
    - `2.7.1 四级多租户架构` (11.0d) / `2.7.2 四级分层RBAC` (3.5d) / `2.7.3 细粒度数据隔离` (5.0d) / `2.7.4 组织Key管理` (1.5d) / `2.7.5 登录安全与角色强制2FA` (4.5d) [P2] / `2.7.6 子账号批量开户` (5.0d)
...
- **里程碑节点 (M1)**：**T+28d (第 4 周末)** 完成 MVP 500 QPS 压测调优与联调测试，正式上线运营。

#### 2. 阶段二：P0 级双端终端应用与高级算力攻坚 (T+29d ~ T+55d，需 27 天，共 82.0 人日)
- **优先级定位**：**P0 (全终端覆盖)** + **P2 (对话导出/静默更新)**
- **交付目标**：在 **T+55d (第 8 周末)** 完成 Win/Mac 双端桌面客户端、网页版对话机器人（Web Chat）以及核心网关专属号池亲和路由与向量语义缓存等高难度算力研发。
- **交付功能模块与子项清单 (与第 2 章节序号精准对应)**：
  - **模块 2.2 一键部署客户端 (Win/Mac 双端)** (共 25.0d) [P0/P2]
    - `2.2.1 托盘常驻` (4.0d) / `2.2.2 节点快捷切换` (5.0d) / `2.2.3 环境一键注入` (4.5d) / `2.2.4 延迟测速大盘` (4.0d) / `2.2.5 静默更新` (3.5d) [P2] / `2.2.6 排查抓包` (4.0d) [P2]
  ...
  - **模块 2.5 核心模型网关 (高级算法攻坚)** (小计 34.0d 算力攻坚) [P0]
    - `2.5.2 渠道专属号池与亲和路由` (18.0d) / `2.5.4 语义缓存降本策略 (Vector 向量检索)` (16.0d)
- **里程碑节点 (M2)**：**T+55d (第 8 周末)** 完成双端客户端与 Web Chat 发布及高级算力验收。

#### 3. 阶段三：P1 级官网门户、国际化与下游代理管理系统 (T+56d ~ T+74d，需 19 天，共 57.0 人日)
- **优先级定位**：**P1 (商业化扩展与分销增长)**
- **交付目标**：在 **T+74d (整整 74 天/第 11 周末)** 完成官网门户搭建、全站中/英国际化多语言、下游代理管理系统（独立 Agent 看板、阶梯销售佣金、分销）以及生产环境部署脚本与交付培训。
- **交付功能模块与子项清单 (与第 2 章节序号精准对应)**：
  - **模块 2.4 官网门户与文档中心** (共 15.0d) [P1]
    - `2.4.1 官网首页` (4.0d) / `2.4.2 模型广场` (3.0d) / `2.4.3 文档中心` (3.0d) / `2.4.4 定价对比矩阵` (3.0d) / `2.4.5 关于与客服` (2.0d)
  - **模块 2.9 国际化多语言 (i18n)** (共 8.0d) [P1]
    - `2.9.1 前端UI全量国际化` (2.5d) / `2.9.2 i18n语言包热更新` (2.0d) / `2.9.3 时区与多币种适配` (2.0d) / `2.9.4 语种自动感知` (1.5d)
  - **模块 2.10 下游代理管理系统** (共 25.0d) [P1]
    - `2.10.1 代理注册分级` (3.0d) / `2.10.2 专属号池分配` (4.5d) / `2.10.3 专属费率加价` (4.0d) / `2.10.4 独立Agent看板` (4.0d) / `2.10.5 下级客户管理` (4.5d) / `2.10.6 阶梯销售佣金结算` (5.0d)
  - **模块 2.11 生产环境部署与运维培训 (阶段三交付项)** (小计 9.0d)
    - `2.11.3 系统部署脚本与生产环境搭建` (4.5d)
    - `2.11.4 运维监控告警部署与上线交付培训` (4.5d)
- **里程碑节点 (M3)**：**T+74d (整整 74 天)** 完成全系统终验交接、运维培训与全面交付。
